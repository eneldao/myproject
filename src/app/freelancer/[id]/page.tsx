        ...
        <div className="bg-[#0a192f]/40 rounded-lg p-6 text-center">
          ...
          <div className="flex items-center justify-center mb-4">
            {[...Array(5)].map((_, i) => (
              <FaStar
                key={i}
                className={i < Math.floor(profile.rating) ? "text-yellow-400" : "text-gray-400"}
              />
            ))}
            <span className="ml-2 text-white">{profile.rating}</span>
          </div>
        ...
        <div className="md:col-span-2 space-y-8">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <ServiceSection
                title="Voice Over"
                items={profile.services.voiceOver}
              />
              <ServiceSection
                title="Conference Interpretation"
                items={profile.services.conferenceInterpretation}
              />
            </div>
          </div>
        </div>
